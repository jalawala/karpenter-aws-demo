...
def RunEc2SpotCustomScheduler():
    ...
    for deploymentData in Ec2SpotK8SCustomSchedulingData:
        ...
        NumOfOnDemandPodsToBeRunning = deploymentData['NumOfOnDemandPodsToBeRunning']
        NumOfSpotPodsToBeRunning = deploymentData['NumOfSpotPodsToBeRunning']
        ...
        lifecycle = 'OnDemand'
        OnDemandNodes = get_node_available_nodes_list(lifecycle)
        NumOfOnDemandPodsRunningAlready =[]
        OnDemandPodsRunningList =  []
        Ec2SpotPodsRunningList =  []
        
        for podRunning in runningPodsList:
            if podRunning['node_name'] in OnDemandNodes.keys():
                OnDemandPodsRunningList.append(podRunning)
            else:
                Ec2SpotPodsRunningList.append(podRunning)

        NumOfOnDemandPodsRunningAlready = len (OnDemandPodsRunningList)
        NumOfEc2SpotPodsRunningAlready  = len (Ec2SpotPodsRunningList)
        NumOfPodsPending  = len (pendingPodsList)
        NumOfPodsFailed  = len (failedPodsList)
        
        
        print("NumOfOnDemandPodsRunningAlready={} OnDemandPodsRunningList={} NumOfEc2SpotPodsRunningAlready={} Ec2SpotPodsRunningList={} NumOfPodsPending={} pendingPodsList={}".format(NumOfOnDemandPodsRunningAlready, OnDemandPodsRunningList, NumOfEc2SpotPodsRunningAlready, Ec2SpotPodsRunningList, NumOfPodsPending, pendingPodsList))
        
        if NumOfOnDemandPodsRunningAlready == NumOfOnDemandPodsToBeRunning:
            print("NumOfOnDemandPodsRunningAlready == NumOfOnDemandPodsToBeRunning = {}. So no need to Schedule".format(NumOfOnDemandPodsRunningAlready))
        elif NumOfOnDemandPodsRunningAlready < NumOfOnDemandPodsToBeRunning:
            NumOfOnDemandPodsToBeScheduled = NumOfOnDemandPodsToBeRunning - NumOfOnDemandPodsRunningAlready
            for i in range(NumOfOnDemandPodsToBeScheduled):
                pod = pendingPodsList[0]
                print("Trying to schedule i={} pod={} with cpu_req={} mem_req={}".format(i, pod['name'], pod['cpu_req'], pod['mem_req']))
                
                for node, stats in OnDemandNodes.items():
